...
Vector2 draw_rotate_red(Vector2 start, Vector2 end, float angle, float scale) {
...
float rotate_right = 55.0f * DEG2RAD;
float rotate_left  = 305.0f * DEG2RAD;
float shrink       = 0.70f;

void render_line(int x1, int y1, int depth, float shrink) {
    if (depth == 0) {
        return;
    }
    Vector2 line_end   = {x1, y1};
    Vector2 line_start = {x1, y1 / 2};
    Vector2 left       = draw_rotate_blue(line_start, line_end, rotate_left - 30.0f, shrink);
    Vector2 right      = draw_rotate_red(line_start, line_end, rotate_right + 30.0f, shrink);
    render_line(left.x, left.y, depth - 1, shrink * 0.5f);
    render_line(right.x, right.y, depth - 1, shrink * 0.5f);
}
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        render_line(WIDTH / 2, HEIGHT / 2, 5, shrink);
        EndDrawing();
    }
